...
import { Calendar, CheckCircle, XCircle, Clock, Gift, Star } from 'lucide-react';
...
export const WeekProgress: React.FC<WeekProgressProps> = ({ weekData, title = "This Week" }) => {
  ...
  const getDayStatus = (entry: any) => {
    const afternoonStatus = entry.afternoon_checkin ? 
      (entry.afternoon_had_drink ? 'bad' : 'good') : 'pending';
    const eveningStatus = entry.evening_checkin ? 
      (entry.evening_had_drink ? 'bad' : 'good') : 'pending';
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6">
      ...
      {/* Weekly Reward */}
      {weekData.reward && (
        <div className={`mb-6 p-4 rounded-xl bg-gradient-to-r ${weekData.reward.color} text-white`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className="text-2xl mr-3">{weekData.reward.icon}</div>
              <div>
                <h4 className="font-bold text-lg">{weekData.reward.title}</h4>
                <p className="text-sm opacity-90">{weekData.reward.description}</p>
              </div>
            </div>
      ...
      <div className="mb-6">
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div
            className={`h-3 rounded-full transition-all duration-500 ${getProgressColor(weekData.percentage)}`}
      ...
      <div className="grid grid-cols-7 gap-2 mb-4">
        {weekData.entries.map((entry, index) => {
          ...
          return (
            <div
              key={entry.date}
              className="p-3 rounded-lg border-2 border-gray-100 bg-gray-50 transition-all duration-200"
            >
              <div className="text-center">
                ...
                {/* Two check-in indicators */}
                <div className="space-y-1">
                  <div className="flex justify-center items-center">
                    <div className="text-xs text-gray-500 mr-1">3PM</div>
                    {getStatusIcon(afternoonStatus, 14)}
                  </div>
                  <div className="flex justify-center items-center">
                    <div className="text-xs text-gray-500 mr-1">8:45PM</div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {14 - weekData.totalCheckins > 0 && `${14 - weekData.totalCheckins} check-ins remaining`}
            </p>
          </div>
          
          {weekData.isComplete && weekData.percentage >= 70 && !weekData.reward && (
